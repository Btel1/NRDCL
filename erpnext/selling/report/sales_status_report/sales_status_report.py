...
def get_data(query, filters):
	data = []
	for d in frappe.db.sql(query, as_dict =1, debug =1):
...
def get_query(filters):
	...
	#where month(pr_detail.pr_posting) = '1'
	return query
...
def get_conditions(filters):
        ...
        if filters.get("month"):
                ...
                filters["month"] = month
        	conditions += " month(pr_detail.pr_creation = '{0}'".format(filters["month"])
        if filters.get("fiscal_year"): conditions += " and year(pr_detail.pr_creation) = '{0}'".format(filters.get("fiscal_year"))
	if filters.get("cost_center"): conditions += " and pr_detail.pr_branch =  '{0}'".format(filters.get("cost_center"))
        return conditions
