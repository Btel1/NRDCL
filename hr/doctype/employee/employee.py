...
class Employee(Document):
	...
	def validate(self):
		...
		if self.passport_number:
			data = frappe.db.sql("select employee_name from `tabEmployee` where docstatus=1 and passport_number = '{0}' and name != '{1}'".format(self.passport_number, self.employee), as_dict=True)
			if data:
	...
	def validate_employment_status(self):
		data = frappe.db.sql("Select name from `tabEmployee Disciplinary Record` where docstatus=1 and not_guilty_or_acquitted=0 and employee = '{0}' and '{1}' between from_date and to_date".format(self.name, today()), as_dict=True)
		if data and self.employment_status != "Suspended":
			frappe.throw("Employement status change not allowed as employee {0} is still under suspension  ref: {1} ".format(self.employee, data[0].name))
...
def get_employee_passport_number(passport_no):
        data = frappe.db.sql("select employee_name from `tabEmployee` where docstatus='1' and passport_number = '{0}'".format(passport_no), as_dict=True)
        if data:
