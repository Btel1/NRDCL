...
frappe.ui.form.on('DES Employee', {
	...
	date_of_joining: function(frm){
		dj = new Date(frm.doc.date_of_joining);
		dj_year = this.dj.getFullYear();
		dj_month = this.dj.getMonth();
		dj_day = this.dj.getDate();
		//To go to same day next year
		expiry_date = new Date(this.dj_year + 1, this.dj_month, this.dj_day);
		cur_frm.set_value("expiry_date", expiry_date);
	}
});
